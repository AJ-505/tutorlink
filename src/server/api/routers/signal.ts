...
export const signalRouter = createTRPCRouter({
  ...
  deleteSignal: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (signal.studentId !== ctx.user.id) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "You can only cancel your own signals",
        });
      }
      if (signal.status !== "pending") {
        throw new TRPCError({
          code: "CONFLICT",
          message: "Only pending signals can be cancelled",
        });
      }
  ...
  acceptSignal: protectedProcedure
    .input(z.object({ signalId: z.string() }))
    .mutation(async ({ ctx, input }) => {
      // Guard: Only tutors can accept signals - query by Clerk ID for consistency
      ...
      if (me?.role !== "TUTOR") {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Only tutors can accept signals",
        });
      }
      ...
      if (signal.status !== "pending") {
        throw new TRPCError({
          code: "CONFLICT",
          message: "Signal already handled",
        });
      }
      ...
      const conversation = await ctx.db.conversation.create({
        data: {
          User: {
            connect: [{ id: me.id }, { id: signal.studentId }],
          },
        },
      });
